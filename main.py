...
def load_image(name):  # если файл не существует, то выходим
...
def start_screen(screen):
    ...
    fon = pygame.transform.scale(load_image('background.png'), (WINDOW_WIDTH, WINDOW_HEIGHT))
    screen.blit(fon, (0, 0))
...
class Hero(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.y = WINDOW_HEIGHT - self.image_bear.get_height() - self.image_bear.get_height() * 0.2
...
def show_game_res(screen, message, score):
    ...
    for line in intro_text:
        string_rendered = font.render(line, 1, color_1)
        intro_rect = string_rendered.get_rect()
    ...
    font = pygame.font.Font(None, int(100 * WINDOW_RATIO))
    text = font.render(message, 1, color_1)
    text_x = WINDOW_WIDTH // 2 - text.get_width() // 2
    text_y = WINDOW_HEIGHT * 0.5 - text.get_height() // 2
    text_w = text.get_width()
    text_h = text.get_height()
    pygame.draw.rect(screen, color_2, (text_x - 10, text_y - 10, text_w + 20, text_h + 20), 5)
    screen.blit(text, (text_x, text_y))
...
def main():
    ...
    image_pine = load_image("shishka.png")
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.USEREVENT and flag:
                ...
                if random.choice([0, 1, 0, 0]):
                    Pine(hero.get_position()[0] + hero.width // 2, random.randint(1, WINDOW_HEIGHT // 2),
                         image_pine, pins)
                    RSN -= 2 * SPEED / 5
                    RSN = max(RSN, 0)
        ...
        if flag:
            ...
            if HLTH == 0:
                screen.blit(image_background, (0, 0))
                show_game_res(screen, 'Game Over', str(score))
            elif RSN == 0:
                show_game_res(screen, 'Victory', str(score))
            else:
                start_screen(screen)
